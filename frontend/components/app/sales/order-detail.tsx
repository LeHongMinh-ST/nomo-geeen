...
function loadErrorTitle(error: unknown): string {
	...
	if (status === 404) return "Không tìm thấy đơn";
...
export function OrderDetail({ id }: { id: string }) {
	...
	const complete = async () => {
		...
		try {
			...
			setActionError(mapSalesApiError(e, "Không thể hoàn thành đơn"));
		} finally {
			setSubmitting(false);
		}
	};
	...
	const cancel = async () => {
		...
		try {
			...
			if ((e as { status?: number })?.status === 409) {
				...
				setActionError(mapSalesApiError(e, "Không thể hủy đơn"));
			}
		} finally {
			setSubmitting(false);
		}
	...
	if (error || !order) {
		return (
			...
			>
				<h1 className="text-lg font-semibold">{loadErrorTitle(error)}</h1>
				<button
					type="button"
	...
	return (
		<div className="mx-auto flex w-full max-w-2xl flex-col gap-5 pb-28">
			...
			{conflict && (
				<div
					role="alert"
					className="rounded border border-[#e6a817] bg-[#fff8e1] p-3"
				>
					Đơn đã thay đổi trên máy chủ. Vui lòng kiểm tra trạng thái mới trước
					khi tiếp tục.
				</div>
			)}
			{actionError ? (
				<div
					role="alert"
					className="rounded border border-destructive bg-[#fff5f5] p-3 text-base text-destructive"
				>
					{actionError}
				</div>
			) : null}
			...
			<section className="rounded border p-5">
				<h2 className="font-semibold">Hàng hóa ({order.lines.length})</h2>
					...
					{order.lines.map((l) => (
						<li key={l.id} className="flex items-center gap-3 py-3">
							...
							<span className="flex-1">
								{l.productName}
								<small className="block text-[#616161]">
									{formatVND(l.unitPrice)}₫ × {l.qty} {l.unitName}
								</small>
							</span>
							<b>{formatVND(l.lineTotal)}₫</b>
						</li>
					))}
				</ul>
			</section>
			<section className="rounded border p-5">
				<div className="flex justify-between">
					Tạm tính <b>{formatVND(order.subtotal)}₫</b>
				</div>
				<div className="flex justify-between">
					Chiết khấu <b>−{formatVND(order.discountAmount)}₫</b>
				</div>
			...
			{order.status === "DRAFT" ? (
				<button
					type="button"
					disabled={submitting}
					className="h-12 rounded bg-primary px-5 text-white"
			...
			{order.status !== "CANCELLED" &&
				(confirm ? (
					<div className="flex gap-3">
						<p className="flex-1">
							Hủy đơn sẽ hoàn tồn kho và điều chỉnh công nợ.
						</p>
						<button
							type="button"
						...
						<button
							type="button"
							disabled={submitting}
							className="h-12 rounded bg-destructive px-4 text-white disabled:opacity-50"
							onClick={() => void cancel()}
						>
							{submitting ? "Đang hủy…" : "Hủy đơn"}
						</button>
					</div>
				) : (
					<button
						type="button"
						className="flex h-12 items-center justify-center gap-2 rounded border border-destructive text-destructive"
