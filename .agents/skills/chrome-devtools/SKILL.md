...
SKILL_DIR=""
if [ -d "packages/spec/src/Codex/skills/chrome-devtools/scripts" ]; then
  SKILL_DIR="packages/spec/src/Codex/skills/chrome-devtools/scripts"
elif [ -d "$HOME/packages/spec/src/Codex/skills/chrome-devtools/scripts" ]; then
  SKILL_DIR="$HOME/packages/spec/src/Codex/skills/chrome-devtools/scripts"
fi
...
node "$SKILL_DIR/aria-snapshot.js" --url https://example.com --output ./packages/spec/src/Codex/chrome-devtools/snapshots/page.yaml
```
...
Skills can exist in **project-scope** or **user-scope**. Priority: project-scope > user-scope.
Store snapshots for analysis in `<project>/packages/spec/src/Codex/chrome-devtools/snapshots/`:

```bash
# Create snapshots directory
mkdir -p packages/spec/src/Codex/chrome-devtools/snapshots

# Capture and store with timestamp
SESSION="$(date +%Y%m%d-%H%M%S)"
node "$SKILL_DIR/aria-snapshot.js" --url https://example.com --output packages/spec/src/Codex/chrome-devtools/snapshots/$SESSION.yaml
```
...
Skills can exist in **project-scope** or **user-scope**. Priority: project-scope > user-scope.
All in `packages/spec/src/Codex/skills/chrome-devtools/scripts/`:
...
Skills can exist in **project-scope** or **user-scope**. Priority: project-scope > user-scope.
For complex automation, write scripts to `<project>/packages/spec/src/Codex/chrome-devtools/tmp/`:

```bash
# Create tmp directory for test scripts
mkdir -p $SKILL_DIR/packages/spec/src/Codex/chrome-devtools/tmp

# Write a test script
cat > $SKILL_DIR/packages/spec/src/Codex/chrome-devtools/tmp/login-test.js << 'EOF'
...
# Run the test
node $SKILL_DIR/packages/spec/src/Codex/chrome-devtools/tmp/login-test.js
```
...
Store screenshots for analysis in `<project>/packages/spec/src/Codex/chrome-devtools/screenshots/`:

```bash
# Basic screenshot
node "$SKILL_DIR/screenshot.js" --url https://example.com --output ./packages/spec/src/Codex/chrome-devtools/screenshots/page.png

# Full page
node "$SKILL_DIR/screenshot.js" --url https://example.com --output ./packages/spec/src/Codex/chrome-devtools/screenshots/page.png --full-page true

# Specific element
node "$SKILL_DIR/screenshot.js" --url https://example.com --selector ".main-content" --output ./packages/spec/src/Codex/chrome-devtools/screenshots/element.png
```
...
node "$SKILL_DIR/screenshot.js" --url https://example.com --output ./packages/spec/src/Codex/chrome-devtools/screenshots/page.png --max-size 3

# Disable compression
node "$SKILL_DIR/screenshot.js" --url https://example.com --output ./packages/spec/src/Codex/chrome-devtools/screenshots/page.png --no-compress
```
...
Store logs for analysis in `<project>/packages/spec/src/Codex/chrome-devtools/logs/<session>/`:

```bash
# Create session directory
SESSION="$(date +%Y%m%d-%H%M%S)"
mkdir -p packages/spec/src/Codex/chrome-devtools/logs/$SESSION

# Capture and store
node "$SKILL_DIR/console.js" --url https://example.com --duration 10000 > packages/spec/src/Codex/chrome-devtools/logs/$SESSION/console.json
node "$SKILL_DIR/network.js" --url https://example.com > packages/spec/src/Codex/chrome-devtools/logs/$SESSION/network.json

# View errors
jq '.messages[] | select(.type=="error")' packages/spec/src/Codex/chrome-devtools/logs/$SESSION/console.json
```
...
node "$SKILL_DIR/screenshot.js" --output ./packages/spec/src/Codex/skills/chrome-devtools/screenshots/debug.png
