...
1. `.codex/runtime.json` → `develop.parallel` is not `false` (escape hatch; missing
...
**Single writer per file per wave** is absolute. targeted file reads rows never conflict.
...
## 3. Dispatch (one god_developer per wave task)
...
git worktree remove --force .codex/worktrees/agent-<id>   # NOT auto-cleaned —
