---
name: hapo:delegate
description: "Delegate implementation tasks from Codex to external CLI agents (Codex or Grok). Covers task-brief authoring, non-interactive dispatch, permission setup, background monitoring, resume, and independent verification of the returned work."
user-invocable: true
...
Dispatch a well-scoped implementation task to an external agentic CLI — **Codex** (OpenAI) or **Grok** (xAI Grok Build) — from a Codex session acting as orchestrator. Codex stays responsible for: writing the task brief, choosing safe permissions, monitoring, and **independently verifying** the result before reporting completion.
...
- Parallel workstreams: Codex keeps orchestrating while an external agent implements a scoped task.
...
External agents run **fresh-context** — they see none of your conversation. Write the brief to a file (convention: `plans/<slug>-task.md` in the target repo), never inline-only. Follow the orchestrator prompt rules (`.Codex/rules/orchestrator.md`): the brief must be self-contained.
...
## Constraints    — what must NOT be touched (staging data, .env, deploys,
                    unrelated refactors); repo rules to obey (AGENTS.md)
...
- Apply the 3-strike rule from `.Codex/rules/orchestrator.md`: same failure 3+ times → stop, escalate to user with evidence.

### 5. Verify independently (non-negotiable)

External-agent claims are **not** evidence (AGENTS.md: no completion claim without fresh proof from the current run). After the agent reports done:
...
- If a spec (`specs/<slug>/`) governs the work, follow the Tollgate Protocol (`.Codex/rules/state-sync.md`): update `spec.json` + task files only after verification passes.
