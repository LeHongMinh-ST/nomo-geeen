...
- [x] 1. Replace the seeded list pipeline with server query state.
...
- [x] 2. Map canonical summaries into existing cards and table layouts.
...
- [x] 3. Load real detail by route ID.
...
- [x] 4. Connect cancellation with confirmation and stale-state recovery.
...
- [x] 5. Add focused list/detail component tests.
...
- [x] Search/status/page issue correct server queries and stale responses never overwrite newer results.
- [x] Desktop replaces pages; mobile appends unique batches and stops at total.
- [x] Detail displays canonical order, line, settlement, debt, and lifecycle data for the route ID.
- [x] Cancel requires confirmation, submits once, and renders the returned DTO or recoverable conflict state.
- [x] Loading, empty, 403/404, error, retry, and success states match DESIGN.md at mobile/tablet/desktop widths.

## Evidence

- [x] Automated verification
  - Command(s): `pnpm --dir frontend test -- order-list.test.tsx order-detail.test.tsx` and `pnpm --dir frontend build`
  - Expected proof: Focused suites and build exit 0. Receipt: `pnpm --dir frontend test -- order-list.test.tsx order-detail.test.tsx` — 12 files / 40 tests passed; `pnpm --dir frontend build` — Next build passed, 43 routes; `git diff --check` passed.
