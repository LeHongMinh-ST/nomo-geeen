...
- [x] 1. Add create/settlement DTOs and guarded routes.
...
- [x] 2. Implement side-effect-free creation and channel-aware idempotency.
...
- [x] 3. Implement shared atomic completion effects.
...
- [x] 4. Add focused unit/controller regression tests.
...
- [x] Draft persists canonical snapshots/note/totals and creates zero stock/debt effects.
- [x] Direct completion and draft completion write exactly one sale effect set and return the same DTO shape.
- [x] Equivalent create/complete retry returns the original state; changed/cross-channel key returns 409.
- [x] Insufficient stock, missing feature, invalid customer/unit/product/payment, or forced write failure leaves order/state/effects unchanged.
- [x] Existing `POST /tenant/sales/quick` remains reachable and its focused tests pass.
...
- `pnpm --dir backend test -- --runInBand sales.service.spec.ts sales.controller.spec.ts`: PASS (2 suites, 11 tests).
- `pnpm --dir backend build`: PASS.
- `git diff --check`: PASS.
- Covered: draft zero side effects, completion stock/debt effects, insufficient-stock rollback boundary, idempotency conflict, quick-sale regression, guarded route reachability.
