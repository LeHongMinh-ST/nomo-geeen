...
- [x] 1. Audit service/controller/DTO against the canonical contract
...
- [x] 2. Complete unit coverage for the service
...
- [x] 3. Complete E2E coverage for the tenant contract
...
- [x] Supplier CRUD/search/soft-delete matches the `design.md` canonical contract with no body-derived tenant trust.
- [x] Duplicate `code` returns `409 DUPLICATE_SUPPLIER_CODE`; missing `code`/`name` returns `422 VALIDATION_ERROR`.
- [x] `balance` is never written by this module and serializes as a JSON number.
- [x] Cross-tenant access returns 404 and unauthorized/feature-gated writes return 403, proven by passing tests.
...
- [x] Automated verification (unit + E2E)
  - Command(s): `cd backend && npx jest suppliers.service.spec --runInBand`; `cd backend && NODE_OPTIONS=--experimental-vm-modules npx jest --config ./test/jest-e2e.json tenant-suppliers.e2e --runInBand`
  - Receipt: unit PASS — 1 suite, 8 tests. E2E PASS — 1 suite, 1 test. Backend build PASS. The default `npx jest tenant-suppliers.e2e` was attempted and found no matches because unit Jest roots at `src`; the E2E config is `test/jest-e2e.json`.
