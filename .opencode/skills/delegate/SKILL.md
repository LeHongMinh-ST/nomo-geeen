...
External agents run **fresh-context** — they see none of your conversation. Write the brief to a file (convention: `plans/<slug>-task.md` in the target repo), never inline-only. Follow the orchestrator prompt rules (`.opencode/rules/orchestrator.md`): the brief must be self-contained.
...
- Apply the 3-strike rule from `.opencode/rules/orchestrator.md`: same failure 3+ times → stop, escalate to user with evidence.
...
- If a spec (`specs/<slug>/`) governs the work, follow the Tollgate Protocol (`.opencode/rules/state-sync.md`): update `spec.json` + task files only after verification passes.
