...
10. **Deterministic validator is mandatory.** The final validation verdict MUST be derived from `node .opencode/scripts/validate-spec-output.cjs specs/<feature>`. If that command fails, report FAIL/BLOCKED and list the script output. Do NOT report PASS.
...
3. Run `node .opencode/scripts/validate-spec-output.cjs specs/<feature>` and keep the raw result visible
4. Fail the audit if:
...
Files modified: {list}
Deterministic validator: PASS via `node .opencode/scripts/validate-spec-output.cjs specs/<feature>`
