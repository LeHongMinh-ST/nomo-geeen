...
- **Layer 1 — Structural:** `node .opencode/scripts/validate-spec-output.cjs specs/<feature>` — non-zero = block.
- **Layer 2 — Grounding (MANDATORY):** `node .opencode/scripts/spec-ground.cjs specs/<feature> [--root <work-context>]` — non-zero = block. Verifies every Modify/Delete/Read Related Files path exists or is Created earlier. Layer 1 pass + Layer 2 fail = NOT ready.
...
**Templates:** `spec-state.json`, `requirements-init.md`, `requirements.md`, `design.md`, `research.md`, `task.md`; validator `.opencode/scripts/validate-spec-output.cjs`
