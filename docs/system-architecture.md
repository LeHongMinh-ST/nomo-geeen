...
- `AppShell` owns the tenant mobile bottom navigation as a four-item grid (`Trang chủ`, `Đơn hàng`, `Sổ tay`, `Khác`) without an extra center action. Mobile page content reserves the bottom-nav safe-area height; long forms add the height of any fixed submit bar so the final fields remain reachable and are not covered by navigation.
