...
- Phase-1 tenant RBAC uses fixed `OWNER`/`MANAGER`/`STAFF` permission templates in `backend/src/platform/roles/tenant-role-permissions.ts`: Owner receives the catalog, Manager receives operational/master-data/report/user/handbook mutations, and Staff receives sales/purchase input plus read-only master-data/debt/handbook access. Seed synchronizes both system templates and existing tenant roles exactly, including removal of stale grants.
...
- `AppShell` owns the tenant mobile bottom navigation as a five-column layout: four route/sheet items (`Trang chủ`, `Đơn hàng`, `Sổ tay`, `Khác`) plus the center `+` shortcut to `/ban-nhanh`. Mobile page content reserves the bottom-nav safe-area height; long forms add the height of any fixed submit bar so the final fields remain reachable and are not covered by navigation. Sidebar, bottom navigation, `MoreSheet`, and settings links filter their items from `user.permissions`; hidden menus are only a UX layer and backend guards remain authoritative.
- Product creation keeps category selection tenant-scoped: the product form derives available industry groups from the tenant's purchased catalog; when exactly one group is available it renders as a fixed value, while multiple groups retain the selector. During creation, inventory base unit, stock, and conversions are optional; the product can be completed later from the post-create edit flow. Product conversion rows use the explicit rule `1 conversion unit = factor base units`, default applicability `BOTH`, and support separate `PURCHASE`, `SALE`, or `BOTH` behavior. Products without a base unit are excluded from sale and handbook operational suggestions until configured.
...
- Price tiers remain a separate follow-up slice; product conversions are editable in the post-create form and carry `PURCHASE`, `SALE`, or `BOTH` applicability. The product API exposes core catalog fields and read-only stock quantity. The advanced sales-order lifecycle is now available under `/tenant/sales/orders`; `POST /tenant/sales/quick` remains the separate inventory-only quick-sale shortcut and does not provide order list/detail or cancellation semantics.
...
- Demo handbook seed follows each tenant's enabled `TenantBusinessGroup` set and the tenant's actual product SKUs. `backend/prisma/seed-demo.ts` assigns disease/handbook entries to relevant products, creates a default `DiseaseProtocol` with concrete `DiseaseProtocolItem.productId` recommendations, and only enables crop, seedling, livestock, veterinary, feed, or aquaculture entries when the store profile supports them. Re-running the demo seed replaces pins/protocols for these managed entries idempotently.
- Handbook disease editing uses `DiseaseProtocol` as the single recommendation source: each protocol item must point to a catalog `Product` selected through the searchable product picker. The former free-form active-ingredient and duplicate quick-suggestion UI is no longer edited or shown; the API keeps legacy response fields for compatibility.
...
- Frontend header `NotificationBell` and `/thong-bao` subscribe to the stream, refresh unread/list on change/reconnect, keep polling fallback while stream is down, and clean up on unmount. Clicking a notification marks it read when needed while routing by type: `DEBT_DUE` → `/cong-no`, `LOW_STOCK`/`NEAR_EXPIRED` → `/ton-kho`, `SYSTEM` → `/thong-bao`. API `readAt` on each item is the **current viewer's** receipt timestamp only.

## QuickSale server-side draft (multi-device sync)

- Module `backend/src/platform/sales/` exposes a server-side POS counter draft so the phone and the desktop share the same cart even though Phase 1 only ships a single-device Zustand store. Models `QuickSaleDraft`, `QuickSaleDraftLine`, `QuickSaleDraftMutation` (`@@unique([draftId, idempotencyKey])`) are tenant-scoped; UUID ids; server computes money via `BigInt`. Migration `20260729120000_quick_sale_draft` extends the `AuditAction` enum with `QUICK_SALE_DRAFT_{CREATE,UPDATE,CLOSE,CHECKOUT}`. Feature remains `inventory`-gated.
- Authorization reuses the existing chain (`TenantAccessTokenGuard` → `TenantPermissionGuard` → `EntitlementsGuard`). Mutations require `sales:create` (lines/checkout) or `sales:edit` (PATCH/DELETE/close). Join (`POST /tenant/sales/quick-draft/join`) only needs `sales:view`; the 10-char alphanumeric `joinToken` is opaque, tenant-local-unique, and verified server-side so cross-tenant scans cannot land on the wrong cart. Phone and desktop must belong to the same tenant.
- One desktop owns at most one active draft at a time; `createOrReactivate` soft-closes any prior open draft for the same owner before issuing a new token. Mutations are idempotent on the `(draftId, idempotencyKey)` pair — replayed calls return the persisted response snapshot; concurrent updates from desktop + phone follow last-write-wins by `updatedAt` (no optimistic lock in Phase 1; detect conflicts via the response rev/replay mismatch).
- 20-minute inactivity expiry is enforced lazily on every read/mutate: `expiresAt = lastTouchedAt + 20m`; an expired draft returns `410 Gone` (`DRAFT_EXPIRED`). No background cron; `QuickSaleDraftMutation` rows survive until the parent draft is explicitly closed (`DELETE /:id`) or checked out (`POST /:id/checkout`).
- Checkout delegates to the existing `SalesService.createQuickSale` so atomicity, FEFO, stock mutation, debt-ledger, audit `SALE_QUICK`, and `Sale.idempotencyKey` semantics stay single-source-of-truth. After the Sale row exists the draft is `closedAt`-stamped and broadcast as `quick-sale-draft.changed` `action=checked-out`. No draft row is hard-deleted on the checkout path; clients see the empty `lines[]` via `GET /current`.
- SSE pipeline mirrors `NotificationEventsService` (`/home/minhlh-hapo/code/nomo-geeen/backend/src/platform/sales/quick-sale-draft-events.{ts,service.ts}`): in-process `Map<id, {tenantId, draftId, listener}>` + Redis pub/sub channel `nomo:tenant-quick-sale-draft`. Endpoint `GET /tenant/sales/quick-draft/:id/stream` emits `connected`, `heartbeat` (~25s), and `quick-sale-draft.changed` (action ∈ `created|line-added|line-quantity-set|line-removed|customer-set|expired|closed|checked-out`). Bearer fetch streaming (no `EventSource`); FE parser in `frontend/lib/use-tenant-quick-sale-draft-stream.ts` reuses the auto-reconnect/backoff pattern from `tenant-notification-stream.ts`. List/join/get stays the source of truth — SSE is a hint to re-fetch.
- FE wiring (`frontend/lib/use-quick-sale-draft.ts`, `frontend/lib/tenant-quick-sale-draft-api.ts`): one hook returns `{draft, status, error, joinToken, addProduct, setQty, removeLine, setCustomer, checkout, close, refresh}`. Bootstraps via `GET /current` when available; otherwise `POST /create` for the desktop (idempotent: reuses an open draft) or `POST /join` for the phone when `?join=…` is in the URL. Component `frontend/components/app/sales/quick-sale-draft-token.tsx` renders the join token with a copy-to-clipboard button. Existing single-device `useQuickSaleStore` (`frontend/stores/quick-sale-store.ts`) is preserved as the offline/local fallback so disconnected tabs still let the cashier close a sale when the server is unreachable.
- Open items deferred to a later slice: QR image generation (current Phase 1 ships only the alphanumeric text token), tenant-scoped foreground sweep for expired drafts, explicit `sales:join` permission code, audit allow/deny logging on join attempts, and payload-fingerprint idempotency conflict surfacing (today a same-key replay returns the cached response; mismatched payloads would need a follow-up patch).
...
- Operational reports expose tenant-scoped stock and completed-sales summaries with
  ...
  tenant dashboard. Stock and sales report endpoints additionally require `report:view` alongside their domain read permission. The `/bao-cao` frontend uses the shared tenant error mapper and keeps
...
- Read-only operational reports are exposed at `/tenant/reports/stock-summary` and
  `/tenant/reports/sales-summary`; both are tenant-scoped and require `report:view` plus the matching
  `inventory:view`/`sales:view` permission and entitlement. The first slice returns stock/batch expiry data and bounded completed
...
- Backend `start:dev` và `start:debug` chạy `prisma generate` trước Nest watch để generated Prisma Client luôn đồng bộ với `backend/prisma/schema.prisma`; `build` và `test` vẫn giữ pre-hook tương ứng.
...
- Backend ghi JSON request/error logs (không ghi token, mật khẩu hoặc PII), đếm `http_requests_total`/`http_errors_total`, và xuất Prometheus text tại `GET /metrics`. Endpoint yêu cầu `X-Metrics-Token` hoặc `Authorization: Bearer ...` khớp `METRICS_TOKEN`; production thiếu token cấu hình sẽ fail closed. Prometheus scrape trực tiếp service nội bộ `backend:3001/metrics` với header token từ secret store. Alert khi error rate, readiness `down`, hoặc readiness `degraded` kéo dài.
- Production Compose không khởi chạy Nginx gateway; backend và frontend expose lần lượt `8005` và `3005` để Cloudflare Tunnel được quản lý bên ngoài server route trực tiếp tới từng service. App guard Redis vẫn là defense-in-depth. `TRUST_PROXY` mặc định `false`; nếu ingress server-side cần IP client thật, phải cấu hình `TRUST_PROXY` theo proxy CIDR đáng tin cậy.
