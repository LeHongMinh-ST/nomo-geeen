...
Fully weaponized with the task creation tool. Broadcast multiple task creation commands AS EARLY AS POSSIBLE, granting robust execution authority to each spawned Sub-Agent.
...
```json
// task creation Command 1: Assign to Database Analysis Agent
{
  ...
  "ExpectedOutput": "A concise text summary published via task-state updates."
}

// task creation Command 2: Assign to Frontend Performance Agent
{
...
// task creation Command 3: Assign to Infrastructure/CI Pipeline Agent
{
...
- **Context Injection (Knowledge Passing):** Do not expect your proxy agents to derive full systemic context from nothing. Inject rich knowledge matrices directly into the Description string during task creation: "Navigate exclusively into directory X, execute command Y, locate error line Z. Return report immediately here".
- **Acknowledge & Gather Pattern (Re-assimilation):** After deploying the fleet, aggressively monitor the grid using task-state inspection or task detail inspection. Wait for operative `DONE` statuses and updates. Finally, amalgamate returning reports into a master Cross-correlation diagnostic map before passing your final judgment execution.
