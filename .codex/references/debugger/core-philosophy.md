...
- **Mandatory:** Read the specific file first (targeted file reads), search the log files (`rg`), or execute bash diagnostic scripts (`exec_command`). You must have absolute certainty of the exact line of code causing the failure before writing the fix.
